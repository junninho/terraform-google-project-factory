...
variable "random_project_id_string_method" {
  description = "Uses a `random_string` for a larger collusion domain than the default `random_project_id` method.  When using this module as part of CI, it is recommended to enable this option rather than `random_project_id`."
  type        = bool
  default     = false
}

variable "random_project_id_length" {
  description = "Length of the `random_project_id_string_method` suffix as integer.  Defaults to 4."
  type        = number
  default     = 4
}
